...
class BusScenario(object):
    ...
    def beginning_values(simulation: typhoon_automator.Simulation):
        ...
        simulation.set_contactor(BusScenario.AtoB, True, False)
        simulation.set_contactor(BusScenario.BtoC, True, False)
        simulation.set_contactor(BusScenario.AtoC, True, False)
        simulation.set_contactor(BusScenario.AtoGnd, True, False)
        simulation.set_contactor(BusScenario.BtoGnd, True, False)
        simulation.set_contactor(BusScenario.CtoGnd, True, False)
    ...
    def flip_switch(simulation: typhoon_automator.Simulation, swControl: bool, swState: bool):
        match int(simulation._fault):
            ...
            case 1:  # Line to Line (A to B)
                simulation.set_contactor(BusScenario.AtoB, swControl, swState)
            case 2:  # Line to Line (B to C)
                simulation.set_contactor(BusScenario.BtoC, swControl, swState)
            case 3:  # Line to Line (A to C)
                simulation.set_contactor(BusScenario.AtoC, swControl, swState)
            case 4:  # Line to Ground (A to Gnd)
                simulation.set_contactor(BusScenario.AtoGnd, swControl, swState)
            case 5:  # Line to Ground (B to Gnd)
                simulation.set_contactor(BusScenario.BtoGnd, swControl, swState)
            case 6:  # Line to Ground (C to Gnd)
                simulation.set_contactor(BusScenario.CtoGnd, swControl, swState)
    ...
    def close_switch(simulation: typhoon_automator.Simulation):
        BusScenario.flip_switch(simulation,True,True)

    def open_switch(simulation: typhoon_automator.Simulation):
        BusScenario.flip_switch(simulation, True, False)
    ...
    def set_up_scenario(
        ...
        """ Set up the Bus scenaro
            First all necessary values are set up
        """
        BusScenario.beginning_values(simulation)
        signals = [
            "Battery inverter.Va",
            "Battery inverter.Vb",
            "Battery inverter.Vc",
