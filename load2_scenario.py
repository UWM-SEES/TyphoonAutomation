...
class Load2Scenario(object):
    ...
    def beginning_values(simulation: typhoon_automator.Simulation):
        ...
        simulation.set_contactor(Load2Scenario.AtoB, True, False)
        simulation.set_contactor(Load2Scenario.BtoC, True, False)
        simulation.set_contactor(Load2Scenario.AtoC, True, False)
        simulation.set_contactor(Load2Scenario.AtoGnd, True, False)
        simulation.set_contactor(Load2Scenario.BtoGnd, True, False)
        simulation.set_contactor(Load2Scenario.CtoGnd, True, False)
    ...
    def close_switch(simulation: typhoon_automator.Simulation):
        Load2Scenario.flip_switch(simulation,True,True)

    def open_switch(simulation: typhoon_automator.Simulation):
        Load2Scenario.flip_switch(simulation, True, False)
    ...
    def set_up_scenario(
        ...
        """ Set up the Load2 scenaro
            First all necessary values are set up
        """
        Load2Scenario.beginning_values(simulation)
        signals = [
            "Battery inverter.Va",
            "Battery inverter.Vb",
            "Battery inverter.Vc",
